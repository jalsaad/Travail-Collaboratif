import { prisma } from "@/lib/prisma";
import type { Prisma } from "@prisma/client";

export const AuditAction = {
  UPDATE_SCHOOL_INFO: "UPDATE_SCHOOL_INFO",
  REGENERATE_JOIN_CODE: "REGENERATE_JOIN_CODE",
  UPDATE_MEMBER_ROLE: "UPDATE_MEMBER_ROLE",
  REMOVE_MEMBER: "REMOVE_MEMBER",
  UPDATE_MEMBER_PROFILE: "UPDATE_MEMBER_PROFILE",
  UPDATE_TEACHING_INFO: "UPDATE_TEACHING_INFO",
  JOIN_VIA_CODE: "JOIN_VIA_CODE",
  CREATE_PEER_REFERRAL: "CREATE_PEER_REFERRAL",
  JOIN_VIA_PEER_REFERRAL: "JOIN_VIA_PEER_REFERRAL",
  INITIATE_PARTIAL_SCHOOL: "INITIATE_PARTIAL_SCHOOL",
  COMPLETE_PARTIAL_SCHOOL: "COMPLETE_PARTIAL_SCHOOL",
  JOIN_PARTIAL_SCHOOL: "JOIN_PARTIAL_SCHOOL",
  PUBLISH_ANNOUNCEMENT: "PUBLISH_ANNOUNCEMENT",
  UPDATE_ANNOUNCEMENT: "UPDATE_ANNOUNCEMENT",
  DELETE_ANNOUNCEMENT: "DELETE_ANNOUNCEMENT",
  REPUBLISH_ANNOUNCEMENT: "REPUBLISH_ANNOUNCEMENT",
  CANCEL_ANNOUNCEMENT: "CANCEL_ANNOUNCEMENT",
  TOGGLE_DONATIONS_FLAG: "TOGGLE_DONATIONS_FLAG",
  CREATE_SCHOOL: "CREATE_SCHOOL",
  APPROVE_SCHOOL: "APPROVE_SCHOOL",
  REJECT_SCHOOL: "REJECT_SCHOOL",
  DELETE_SCHOOL: "DELETE_SCHOOL",
  UPDATE_PERIOD: "UPDATE_PERIOD",
  DELETE_PERIOD: "DELETE_PERIOD",
  EXPORT_USERS_DIRECTORY: "EXPORT_USERS_DIRECTORY",
  ARCHIVE_SCHOOL_YEAR: "ARCHIVE_SCHOOL_YEAR",
  CREATE_SCHOOL_YEAR: "CREATE_SCHOOL_YEAR",
  UPDATE_SCHOOL_YEAR: "UPDATE_SCHOOL_YEAR",
  ANONYMIZE_ACCOUNT: "ANONYMIZE_ACCOUNT",
  RESET_PASSWORD: "RESET_PASSWORD",
  CREATE_SUPPORT_TICKET: "CREATE_SUPPORT_TICKET",
  RESOLVE_SUPPORT_TICKET: "RESOLVE_SUPPORT_TICKET",
  DELETE_SUPPORT_TICKET: "DELETE_SUPPORT_TICKET",
  UPDATE_PROSPECTION: "UPDATE_PROSPECTION",
} as const;

export async function logAudit(params: {
  // null pour les actions transverses superadmin, non rattachées à une école
  // précise (cf. commentaire de AuditLog.schoolId dans schema.prisma).
  schoolId: string | null;
  actorId: string;
  action: string;
  targetType?: string;
  targetId?: string;
  metadata?: Prisma.InputJsonValue;
}) {
  await prisma.auditLog.create({ data: params });
}
