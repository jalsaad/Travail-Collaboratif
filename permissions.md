...
`/rejoindre` propose un troisième mode, à côté du code de rattachement et de la recherche d'une école déjà inscrite : « Pas encore inscrite », qui cherche dans l'annuaire FWB (comme `/creer-ecole`) et crée l'école avec `School.status = "PARTIAL"` (cf. `app/(auth)/rejoindre/actions.ts::initiatePartialSchool`).
...
- **Email de direction obligatoire** à l'initiation (`School.directionEmail`, `directionNotifiedAt`) : sert uniquement à l'informer par email (`lib/mailer.ts::sendDirectionInvitationEmail`), jamais à créer un compte. L'email invite à inscrire l'école officiellement via `/creer-ecole`, en soulignant la gratuité et la simplification apportées.
