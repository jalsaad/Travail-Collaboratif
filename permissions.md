...
**Fondation par un·e enseignant·e** : `/creer-ecole` propose « Enseignant·e » (option par défaut) au même titre que Direction/Direction adjointe/Autre — un·e enseignant·e n'a plus besoin d'attendre que sa direction s'en charge pour commencer à utiliser la plateforme. Comme « Autre », ce choix donne le rôle `REFERENT_NUMERIQUE` (Admin), jamais `DIRECTION` (cf. `app/(auth)/creer-ecole/actions.ts`). Conséquence à assumer : si la direction rejoint ensuite cette école (par code de rattachement ou parrainage), elle entre comme n'importe quel autre membre — `ENSEIGNANT` ou, si promue par le fondateur, `REFERENT_NUMERIQUE` — jamais `DIRECTION`, ce rôle restant réservé au fondateur (`isAccountOwner`) et non attribuable après coup (cf. `updateMemberRole`, qui l'exclut explicitement). Fonctionnellement sans perte puisqu'Admin a les mêmes droits que Super Admin, mais la direction n'aura pas le statut de titulaire de compte si elle n'est pas la fondatrice.
