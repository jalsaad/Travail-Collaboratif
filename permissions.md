...
**`/creer-ecole` reste réservé à la direction** (Direction/Direction adjointe/Autre — jamais « Enseignant·e ») : se déclarer fondateur d'un espace école, même relabellisé Admin, est mal perçu par les directions quand ça vient d'un enseignant. Un enseignant qui veut réunir son petit cercle de collègues sans attendre passe par son propre formulaire d'inscription (`/rejoindre`, mode « Pas encore inscrite »), qui ne lui attribue jamais de rôle de gestion — voir École PARTIAL ci-dessous.

## École PARTIAL — cercle informel initié par un·e enseignant·e

`/rejoindre` propose un troisième mode, à côté du code de rattachement et de la recherche d'une école déjà inscrite : « Inscription libre », qui cherche dans l'annuaire FWB (comme `/creer-ecole`) et crée l'école avec `School.status = "PARTIAL"` (cf. `app/(auth)/rejoindre/actions.ts::initiatePartialSchool`).

Les **trois chemins sont identiques dans les deux parcours de rattachement** — création de compte (`/rejoindre`) et ajout d'une école à un compte existant (`/rejoindre-ecole`) : mêmes champs (`components/school-choice-fields.tsx`) et mêmes gardes serveur (`lib/school-join-target.ts::resolveExistingSchoolTarget`, `loadFwbSchoolForInitiation`, `createPartialSchoolRecord`). Un enseignant déjà rattaché à une école peut donc aussi initier un cercle PARTIAL dans une seconde école, aux mêmes conditions (rôle `ENSEIGNANT`, email de direction obligatoire).

- **Personne n'y détient de rôle de gestion.** L'enseignant·e qui initie reste `ENSEIGNANT` — jamais `DIRECTION` ni `REFERENT_NUMERIQUE`. Aucun `JoinCode` ne peut donc y être généré (réservé à Admin/Direction) ; la circulation se fait uniquement par parrainage entre pairs (`/inviter`, ouvert à tout membre actif) ou en retrouvant l'école par son nom (`searchJoinableSchools`, qui affiche aussi les écoles PARTIAL, sans code requis pour les rejoindre).
- **Opérationnelle tout de suite**, comme APPROVED (`app/(app)/layout.tsx` ne bloque que PENDING/REJECTED) — aucune validation plateforme requise, le risque étant contenu (aucune autorité institutionnelle n'est réclamée).
- **Email de direction obligatoire** à l'initiation (`School.directionEmail`) : sert uniquement à l'informer par email (`lib/mailer.ts::sendDirectionInvitationEmail`), jamais à créer un compte. L'email invite à inscrire l'école officiellement via `/creer-ecole`, en soulignant la gratuité et la simplification apportées. **Renvoyé à chaque nouveau ralliement** (pas seulement à l'initiation) tant que l'école reste PARTIAL — `joinViaCode` relance la même invitation à chaque enseignant·e supplémentaire qui rejoint via « Chercher mon école », et met à jour `directionNotifiedAt` à chaque envoi.
- **Affichée « Inscrite partiellement »** sur `/admin/ecoles` et la cartographie (`/admin/cartographie`, filtre « Sur la plateforme »), pour distinguer ce cercle informel d'une inscription officielle par la direction (`APPROVED`).
- `initiatePartialSchool` revérifie qu'aucune `School` n'existe déjà pour ce `numeroFase` (peu importe son statut) avant de créer — une école déjà présente doit être rejointe via « Chercher mon école », jamais réinitiée.

**Reprise par la direction** : `/creer-ecole` ne refuse PAS une école déjà présente en statut PARTIAL — c'est exactement la situation que l'email d'invitation demande à la direction de régler. `createSchool` met alors à jour l'école existante au lieu d'en créer une seconde : le cercle, les membres et les périodes déjà déclarées sont conservés, la direction reçoit `DIRECTION` + `isAccountOwner`, et un `JoinCode` est enfin généré (le cercle n'en avait aucun, personne n'y ayant le droit d'en créer). L'école passe directement `APPROVED`, et non `PENDING` comme une création de zéro : elle fonctionnait déjà, et l'attente d'une validation plateforme l'aurait mise à l'arrêt pour des enseignant·es qui n'ont rien demandé (cf. la garde dans `app/(app)/layout.tsx`). La plateforme est prévenue (`notifyPlatformOfNewSchool`) et l'événement est journalisé sous `COMPLETE_PARTIAL_SCHOOL`, distinct de `CREATE_SCHOOL`. Une école dans tout autre statut reste refusée comme doublon.
- **Invitation depuis la déclaration** : dans une école PARTIAL, `/declarer` affiche un champ « Collègues sans compte » (`components/colleague-invites-field.tsx`). Chaque email saisi reçoit, après la création de la période, un `PeerReferral` rattaché à CETTE période (`lib/peer-referrals.ts::invitePeerByEmail`) — créer son compte via ce lien vaut donc confirmation immédiate de la participation. Nécessaire parce que le sélecteur de collègues y est vide au départ et que personne n'y a le droit de générer un `JoinCode` : sans ce champ, la première personne du cercle n'a personne à taguer.
...
- Même invariant de sécurité que `ParticipationToken`/`PasswordResetToken` : seul `tokenHash` (SHA-256) vit en base, le jeton brut ne voyage que dans le lien/QR transmis — soit en main propre par l'enseignant lui-même, soit par un email optionnel envoyé automatiquement à l'adresse du·de la collègue (`inviteeEmail`, cf. `lib/mailer.ts::sendPeerReferralEmail`) si celui-ci·celle-ci la connaît au moment de générer le lien. Conséquence dans les deux cas : un lien non copié/envoyé immédiatement après génération est perdu (pas de réaffichage possible depuis la base), il faut en régénérer un.
